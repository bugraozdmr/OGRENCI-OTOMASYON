...
void ogretmen::ogretmen_menu() {
	int secim, n;
	static double toplam;		//indis olarak kullanýlacagi icin static olarak tanýmlandý
	static int d = -1;			//fonksiyon her çaðýrýlýdýðýnda deðerini korur.
	system("cls");							//yani eðer 1 artarsa atamalarda yeni deðeri 1 artmýþ halidir gibi. !!!
	...
	case '2':
		...
		if (d < 0) {
			...
			toplam *= 0;		//sýfýrlasinki tekrar tekrar çaðýrýldýðýnda sorun olmasýn
			system("pause");
			ogretmen_menu();
		}
	case '3':
...
void gorevli_menu() {
	...
	case '2':
		...
		for (int op = 0; op <= i; op++) {
			if (ogrenciler[op].numara == aranan) {
				for (int k = op; k < i; op++) {
					ogrenciler[k] = ogrenciler[k + 1];		//index boþaltýldý
				}
	...
	case '4':
		...
		cin >> s;			//ogretmen numarasi her giriste otomatik 1 artýyor.
		a++;				//indis 1 arttýrýldý.
	...
	case '5':
		...
		for (int j = 0; j <= a; j++) {
			if (ogretmen[j].ogretnmen_no == aranan) {
				for (int k = j; k < a; k++) {			//en son deðer neyse sürekli eþitlik yapa yapa boþ deðere 
					ogretmen[k] = ogretmen[k + 1];		//eþitliyor yani sýfýrlýyor...
				}
				cout << "Kayit silindi." << endl;
				a--;
				system("pause");
